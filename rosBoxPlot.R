...
#' with P followed by a number.  It determines the associated remark,
#' or qualification columns, based on them having column headings that 
#' start with R followed by numbers that match the associating 
#' concentration data. Then it determines which values are censored, 
#' indicated by a less than symbol in the R columns, performs 
#' regression on order statistics, \link{ros}, using the NADA package, 
#' and estimates values for the censored concentrations.  The 
#' water-quality concentrations are then depicted by boxplots.
#' @name rosBoxPlot
...
#' warning message when interpretting the plots.
...
rosBoxPlot <- function(data, site="", qwcols=c("R", "P"), ...) {
  ...
  for (i in 1:length(parms) ) {
    ...
    my.res[[paste(names[i], sep="")]] <- my.ros$modeled
  }
  boxplot(my.res, ...)
