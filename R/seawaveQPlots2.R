seawaveQPlots2 <- function(stpars, cmaxt, tseas, tseaspr, tndlin, tndlinpr, 
  ...
  # tndrcsprAll <- matrix(data = tndlinpr, nrow = length(tndlinpr), ncol = (numknots - 1))
  ...
  lines(c(xlow + 0.45 * (xup - xlow), xlow + 0.49 * (xup - 
                                                       xlow)), c(yup - 7.5 * ytck, yup - 7.5 * ytck), lwd = 1.5, 
        col = 1)
  ...
  xup <- yup
  xtck <- 0.5 * ytck
  plot(fitadjxdat, cadjx0, type = "p", pch = "", xlim = c(xlow, 
                                                          xup), ylim = c(ylow, yup), xaxs = "i", yaxs = "i", xaxt = "n", 
       yaxt = "n", xlab = "", ylab = "")
